# Run STFactor on spatial transcriptome data
...
# file_name = 'data/Layer2_BC_count_matrix-1.tsv'
STFactor = function(file_name, num_components, output_flag, file_sep = '\t', top_hvg = 2000, gene_filtering = 0.1, loc_sep = 'x'){
    ...
    # check data
    if(any(is.na(exp_mat))){    # check NA in tensor data
    stop('ERROR: the data contains missing data')
    }
    if(any(is.infinite(as.matrix(exp_mat)))){   # check infinite values 
    stop('ERROR: the data contains infinite data')
    }
    if(!is.numeric(exp_mat)){   # check numeric values
    ...
    rownames(loc_loadings) = loc_list
    colnames(loc_loadings) = factor_list
    write.table(loc_loadings, file = paste0('results/', output_flag, '_factor_spatial_patterns.txt'), sep = '\t')

    # spatial pattern visulaization
    ...
    print('Plotting STFactor patterns')
